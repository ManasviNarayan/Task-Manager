"""
Task service for the Task Manager application.

This module contains the TaskService class which implements business logic
for task operations including CRUD, validation, and history tracking.
"""
...
from task_manager.domain.models import History, HistoryType, Status, Subtask, Task
...
class TaskService:
    ...
    validation, CRUD operations, history tracking, and status cascading
    to subtasks.
    """

    def __init__(self, task_uow: ITaskUnitOfWork) -> None:
        """
        Initialize the TaskService with a Unit of Work.
        
        Args:
            task_uow: Unit of Work for data access operations.
        """
    ...
    def get_task(self, task_id: str) -> Task:
        ...
        try:
            ...
            logger.exception(
                "Unexpected error in TaskService.get_task for id %s", task_id
            )
            raise DomainError(f"Service failed to fetch task with id {task_id}") from e

    def get_task_history(self, task_id: str | None = None) -> list[History]:
        """
    ...
    def create_task(self, task: Task) -> Task:
        ...
        """
        try:
            validation_result = pipelines.task_create_validation_pipeline(
                task.deadline
            )
    ...
    def _cascade_status_to_subtasks(
        self, task_id: str, new_status: Status
    ) -> None:
        """
        ...
        Args:
            ...
            new_status: The new status to cascade.
        """
        subtasks = self.task_uow.subtasks.get_subtasks(task_id)
        for subtask in subtasks:
            ...
            logger.info(
                "Cascaded status '%s' to subtask %s (parent task: %s)",
                new_status.value,
                subtask.id,
                task_id,
            )
